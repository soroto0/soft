...
class App(tk.Tk):
    ...
    def run_bg(self, fn, *args, name="Задача"):
        # Одна тяжёлая задача за раз. Раньше блокировался только повтор
        # с тем же именем — «Озвучка» и «Генерация видео» шли параллельно
        # и писали в одни и те же файлы проекта, портя друг друга.
        if not hasattr(self, "_busy_tasks"):
            self._busy_tasks = set()
        if self._busy_tasks:
            running = ", ".join(sorted(self._busy_tasks))
            messagebox.showwarning(
                APP_TITLE, f"Сейчас выполняется: «{running}».\n\n"
                           f"«{name}» не запущена — дождись завершения или "
                           "останови текущую задачу (⛔ Стоп для рендера).\n"
                           "Ход операции виден на странице «Консоль».")
            return
